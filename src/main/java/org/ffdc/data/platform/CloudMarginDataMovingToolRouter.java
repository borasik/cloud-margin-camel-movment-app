...
public class CloudMarginDataMovingToolRouter extends RouteBuilder {
    ...
    public void configure() {
        ...
        // Predicate validationMessage =
        // PredicateBuilder.and(body().contains("validationCode"),
        // body().contains("validationUrl"));
                ...
                .process(new Processor() {
                    public void process(Exchange exchange) throws Exception {
                        ...
                        if (body.contains("blobUrl")) {
                            ...
                            throw new Exception(
                                    "Message Type Received not Recognized (Nor blob message nor validation message). In CloudMarginDataMovingToolRouter");
                        }
                    }
                }).choice()
                .when(exchangeProperty(Is_Azure_DataLake_Validation_Subscription_Message).isEqualTo("false"))
                    .to("direct:get-data-set-from-azure-dl")
                .when(exchangeProperty(Is_Azure_DataLake_Validation_Subscription_Message).isEqualTo("true"))
                    .to("direct:blob-azure-subscription-handshake-response");

        from("direct:get-data-set-from-azure-dl").log("Start Pulling Data Set From ADSL...")
                .routeId("get-data-set-from-azure-dl").unmarshal()
                .json(JsonLibrary.Jackson, AzureBlobCreateBlobEventPayload[].class).process(exchange -> {
                    AzureBlobCreateBlobEventPayload[] azureBlobCreateBlobEventPayload = exchange.getIn()
                            .getBody(AzureBlobCreateBlobEventPayload[].class);
                    // TODO: Try Catch
        ...
        from("direct:blob-azure-subscription-handshake-response").log("Starting Subscription Validation Process...")
                .unmarshal().json(JsonLibrary.Jackson, AzureBlobSubscriptionValidationPayload[].class)
                .process(exchange -> {
                    AzureBlobSubscriptionValidationPayload[] azureBlobSubscriptionValidationPayload = exchange.getIn()
                            .getBody(AzureBlobSubscriptionValidationPayload[].class);
                    // TODO: Try Catch
                ...
                //.setBody()
                .log("BODY: {'validationResponse':" + "'${exchangeProperty.validationCode}'" + "}")
                //.log("Full URI: ${exchangeProperty.validationUrlPath}" + "?${exchangeProperty.query}" + "&bridgeEndpoint=true")
                .endRest();             
                //.toD("${exchangeProperty.validationUrlPath}" + "?${exchangeProperty.query}" + "&bridgeEndpoint=true")
                //.toD("${exchangeProperty.validationUrlPath}&bridgeEndpoint=true");
                ...
                // .toD("https://www.newsru.co.il?bridgeEndpoint=true");
    }
}
